        ...
        for i in range(2):
            ...
            for j in range(3):
                ...
                for e2 in vecEdges:
                    ...
                    # If det is close to 0, the lines are parallel and won't intersect
                    if abs(det) > 0.0001:
                        t2 = (rdx * (e2.sy - oy) + (rdy * (ox - e2.sx))) / det

                        # Prevent division by zero if the ray is perfectly vertical (rdx is 0)
                        if abs(rdx) > 0.0001:
                            t1 = (e2.sx + sdx * t2 - ox) / rdx
                        else:
                            # If rdx is 0, we can safely divide by rdy instead
